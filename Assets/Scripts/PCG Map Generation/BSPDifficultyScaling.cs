...
public class BSPDifficultyScaling : MonoBehaviour
{
    [SerializeField]
    private GameTimer gameTimer;
    [SerializeField]
    private EnemySpawnerBSP enemySpawner;
    ...
    [SerializeField]
    private float finalHealthPercent = 50f;
    ...
    [SerializeField]
    private float finalDamagePercent = 50f;
    
    // enemy count increase for each difficulty spike
    [SerializeField]
    private float firstEnemiesPerRoomPercent = 20f;
    [SerializeField]
    private float secondEnemiesPerRoomPercent = 25f;
    [SerializeField]
    private float finalEnemiesPerRoomPercent = 50f;
    ...
    [SerializeField]
    private float finalRespawnReductionPercent = 50f;
    ...
    [SerializeField]
    private float finalEliteChance = 0.40f;
    ...
    void Update()
    {
        ...
        if (firstTriggered == false && timeRemainingPercent <= firstTriggerPercent)
        {
            ...
            enemySpawner.IncreaseEnemiesPerRoom(firstEnemiesPerRoomPercent);
        ...
        if (secondTriggered == false && timeRemainingPercent <= secondTriggerPercent)
        {
            ...
            enemySpawner.IncreaseEnemiesPerRoom(secondEnemiesPerRoomPercent);
    ...
    // event triggers when timer runs
    void HandleTimerFinished()
    {
        IncreaseEnemyHealth(finalHealthPercent);
        IncreaseEnemyMeleeDamage(finalDamagePercent);
        enemySpawner.IncreaseEnemiesPerRoom(finalEnemiesPerRoomPercent);
        enemySpawner.ReduceRespawnDelay(finalRespawnReductionPercent);
        enemySpawner.SetEliteSpawnChance(finalEliteChance);
    }
